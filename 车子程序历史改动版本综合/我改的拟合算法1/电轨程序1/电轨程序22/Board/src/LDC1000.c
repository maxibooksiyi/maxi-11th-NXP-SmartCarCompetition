...
{
      while(orgVal1[1]!=RPMAX||orgVal1[2]!=RPMIN||orgVal1[3]!=RFREQ)//一旦在此循环说明初始化不成功
      {  
         FLOAT_SPI_init1();   
         FLOAT_delay_us(3000);
         FLOAT_Singal_SPI_Write1(LDC1000_CMD_RPMAX, RPMAX);
         FLOAT_Singal_SPI_Write1(LDC1000_CMD_RPMIN, RPMIN);//0x14
         FLOAT_Singal_SPI_Write1(LDC1000_CMD_SENSORFREQ,  RFREQ);  //谐振频率计算方法见《浮点科技电轨传感器调试手册》
         FLOAT_Singal_SPI_Write1(LDC1000_CMD_LDCCONFIG,   0x17);  /********/
...
long int filter1()//滤波函数，需要认真改写，否则数据跳动，会造成互感的假象
{
     ...
     for(i=0;i<5;i++)//每个采5次
     {   
         ad_valu[i]=ldc_read_avr1();     // ADC1 通道     PTB1       
...
{
      while(orgVal2[1]!=RPMAX||orgVal2[2]!=RPMIN||orgVal2[3]!=RFREQ)//一旦在此循环说明初始化不成功
      {  
         FLOAT_SPI_init2();   
         FLOAT_delay_us(3000);
         FLOAT_Singal_SPI_Write2(LDC1000_CMD_RPMAX, RPMAX);
         FLOAT_Singal_SPI_Write2(LDC1000_CMD_RPMIN, RPMIN);//0x14
         FLOAT_Singal_SPI_Write2(LDC1000_CMD_SENSORFREQ,  RFREQ);  //谐振频率计算方法见《浮点科技电轨传感器调试手册》
         FLOAT_Singal_SPI_Write2(LDC1000_CMD_LDCCONFIG,   0x17);  /********/
...
long int filter2()
{
    int16  i,j,k,temp; 
     ...
     for(i=0;i<5;i++)//每个采5次
     {   
         ad_valu[i]=ldc_read_avr2();     // ADC1 通道     PTB1       
