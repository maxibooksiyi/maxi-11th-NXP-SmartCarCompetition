...
#define NRF_CE_HIGH()	    PTB8_OUT=1
#define NRF_CE_LOW()	    PTB8_OUT=0			  //CE置低
#define NRF_PCSN_LOW()      PTA14_OUT=0
#define NRF_PCSN_HIGH()     PTA14_OUT=1
#define NRF_Read_IRQ()	    PTB9_IN
...
void NRF_Init(void)
{
    ...
    gpio_init(PORTB,8, GPO,LOW);           //初始化CE，默认进入待机模式 
    gpio_init(PORTA,14, GPO,HIGH);          //初始化PCSN管脚为输出，低电平选中  
    
#if IS_USE_ISR
    exti_init(PORTB,9, falling_up);        //初始化IRQ管脚为 :下降沿触发，内部上拉
#else
    gpio_init(PORTB,9, GPI,LOW);           //初始化IRQ管脚为输入     
#endif
